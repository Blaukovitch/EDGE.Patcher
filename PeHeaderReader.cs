	...
	public class PeHeaderReader
	{
		...
		public PeHeaderReader(string filePath)
		{
			...
			using (FileStream stream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
			{
				...
				{
					_optionalHeader32 = FromBinaryReader<IMAGE_OPTIONAL_HEADER32>(reader);
				}
				else
				{
					_optionalHeader64 = FromBinaryReader<IMAGE_OPTIONAL_HEADER64>(reader);
				}
				...
				//DelayLoadTable
				DelayLoadName2FileOffset.Clear();
				var delayLoadTable = (Is32BitHeader ? _optionalHeader32.DelayImportDescriptor : _optionalHeader64.DelayImportDescriptor);
				ReadDelayLoadSection(delayLoadTable, reader);

				//ImportTable
				ImportName2FileOffset.Clear();
				var importTable = (Is32BitHeader ? _optionalHeader32.ImportTable : _optionalHeader64.ImportTable);
				ReadImportSection(importTable, reader);
			}
		}
		...
		public long OptionalHeaderField2FileOffset(string fieldName)
		{
			var fieldOffset = Is32BitHeader 
				? (long)Marshal.OffsetOf(typeof(IMAGE_OPTIONAL_HEADER32), fieldName)
				: (long)Marshal.OffsetOf(typeof(IMAGE_OPTIONAL_HEADER64), fieldName);
			return _dosHeader.e_lfanew + Marshal.SizeOf(typeof(IMAGE_FILE_HEADER)) + 4 + fieldOffset;
		}
		...
		private static UInt32 RVAtoFileOffset(IMAGE_SECTION_HEADER section, UInt32 RVA)
		{
		...
		private IMAGE_SECTION_HEADER DataToSection(IMAGE_DATA_DIRECTORY data)
		{
			foreach (var section in _imageSectionHeaders)
			{
				if (data.VirtualAddress >= section.VirtualAddress && data.VirtualAddress < section.VirtualAddress + section.VirtualSize)
				{
					return section;
				}
			}
		...
		private void ReadImportSection(IMAGE_DATA_DIRECTORY dataDirectory, BinaryReader reader)
		{
			...
			try
			{
				var sectionHeader = DataToSection(dataDirectory);

				reader.BaseStream.Seek(RVAtoFileOffset(sectionHeader, dataDirectory.VirtualAddress), SeekOrigin.Begin);
				var nameRVAs = new List<uint>();
				while (true)
				{
					IMAGE_IMPORT_DIRECTORY_TABLE item = FromBinaryReader<IMAGE_IMPORT_DIRECTORY_TABLE>(reader);
					if (item.ImportLookupTableRVA == 0)
					{
						break;
					}

					nameRVAs.Add(item.NameRVA);
				}

				ReadNames(ImportName2FileOffset, nameRVAs, sectionHeader, reader);
			}
			catch (Exception ex)
			{
				Console.WriteLine("Exception in ReadImportSection: {0}", ex.Message);
			}
		}

		private void ReadDelayLoadSection(IMAGE_DATA_DIRECTORY dataDirectory, BinaryReader reader)
		{
			if (dataDirectory.VirtualAddress == 0)
			{
				return;
			...
			try
			{
				...
				while (true)
				{
					var item = FromBinaryReader<IMAGE_DELAY_LOAD_DIRECTORY_TABLE>(reader);
					if (item.ImportAddressTableRVA == 0)
					{
						break;
					}
				...
				ReadNames(DelayLoadName2FileOffset, nameRVAs, sectionHeader, reader);
			}
			catch (Exception ex)
			{
				Console.WriteLine("Exception in ReadDelayLoadSection: {0}", ex.Message);
			}
		}

		private void ReadNames(Dictionary<string, long> names, IEnumerable<uint> nameRVAs, IMAGE_SECTION_HEADER sectionHeader, BinaryReader reader)
		{
			foreach (var nameRVA in nameRVAs)
			{
				var fileOffset = RVAtoFileOffset(sectionHeader, nameRVA);
				...
				names.Add(Encoding.ASCII.GetString(name).ToLowerInvariant(), fileOffset);
			}
		}
		...
		public Dictionary<string, long> ImportName2FileOffset = new Dictionary<string, long>();


		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		public class IMAGE_DELAY_LOAD_DIRECTORY_TABLE
		{
			//The import directory table consists of an array of import directory entries, one entry for each DLL to which the image refers.The last directory entry is empty(filled with null values), which indicates the end of the directory table.
			...
			public UInt32 HmodRVA;
			public UInt32 ImportAddressTableRVA;
			public UInt32 IntRVA;
			public UInt32 BoundIatRVA;
			public UInt32 UnloadIatRVA;

			//The stamp that is set to zero until the image is bound. After the image is bound, this field is set to the time/data stamp of the DLL.
			public UInt32 TimeStamp;
		}
		public Dictionary<string, long> DelayLoadName2FileOffset = new Dictionary<string, long>();
